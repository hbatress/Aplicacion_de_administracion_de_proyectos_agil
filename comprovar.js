...
function inicarContr(){
    ...
    // Guarda la posición actual en la URL
...
function indexContr() {
    ...
    // Verifica si hay una posición almacenada en la URL y restáurala
    const scrollPosition = sessionStorage.getItem('scrollPosition');
    if (scrollPosition) {
        window.scrollTo(0, parseInt(scrollPosition, 10));
        sessionStorage.removeItem('scrollPosition'); // Limpia la posición almacenada
    }
}
...
document.addEventListener('click', ev => {
    if (ev.target.matches('#crearCuenta')) {
        // Guarda la posición actual en la URL
        const currentScrollPosition = window.scrollY;
        window.location.href = '#crearCuenta'; // Agrega un anclaje a la URL
        sessionStorage.setItem('scrollPosition', currentScrollPosition);
        createContr();
    } else if (ev.target.matches('#IniciarSesion')) {
        // Guarda la posición actual en la URL
        const currentScrollPosition = window.scrollY;
        window.location.href = '#IniciarSesion'; // Agrega un anclaje a la URL
        sessionStorage.setItem('scrollPosition', currentScrollPosition);
        inicarContr();
    } else if (ev.target.matches('#Plataforma')) {
        // Guarda la posición actual en la URL
        const currentScrollPosition = window.scrollY;
        window.location.href = '#Plataforma'; // Agrega un anclaje a la URL
        sessionStorage.setItem('scrollPosition', currentScrollPosition);
        Plataforma();
    }
});
